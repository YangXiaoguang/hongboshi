...
- 当前状态：`ADM-IA-B-C-D 课程商品发布队列只读分组与批量操作预案` 已完成，课程商品中心新增发布队列预案区，可按内容质量、审核状态和上架状态只读分组，并展示批量提交审核/审核跟进/上架/复查预案与安全边界。
- 本轮完成后下一步：执行 `ADM-IA-B-C-E 课程商品发布队列服务端聚合契约与批量草案`
...
- 完成课程商品发布队列只读预案：新增发布队列纯模型和后台面板，按待补内容、待提交审核、待审核、待上架和已上架复查分组，并展示批量提交审核、审核跟进、批量上架和已发布复查的 preview-only 预案，不调用批量写接口。
- 完成心理咨询类项目的现代化界面优化。
...
- 新增 `courseProductPublishQueueModel.ts`，用纯函数按课程商品状态、审核状态和内容质量生成发布队列分组。
- 新增 `CourseProductPublishQueuePanel`，展示待补内容、待提交审核、待审核、待上架和已上架复查 5 个只读队列。
- 课程商品列表额外读取当前筛选范围内最多 200 个商品用于队列预案；超过范围会标记截断，避免前端无界读取大数据。
- 批量预案明确 `previewOnly=true`、`executable=false`，仅展示批量提交审核、批量审核跟进、批量上架和已发布复查的候选/阻塞/风险，不新增写动作。
- 新增模型单元测试，覆盖分组规则、归档商品排除、批量预案不可执行和风险分布。

### 后台待续：ADM-IA-B-C-E 课程商品发布队列服务端聚合契约与批量草案

业务目标：

前端只读预案已经帮助运营看到发布队列，但大规模商品不能长期依赖前端聚合。下一步要把发布队列抽象成服务端聚合契约，并先建立“批量任务草案”而非真实执行：运营可基于筛选条件生成提交审核/上架候选快照，系统保存候选、阻塞、风险、创建人和原因，等待后续二次审批与漂移预检。

建议实施范围：

- 在 `shared/domain` 增加课程商品发布队列摘要、批量草案、候选快照、风险摘要和预览结果契约。
- 新增只读服务端聚合 API，按筛选条件返回队列分组和预案摘要，替代前端最多 200 条的临时聚合边界。
- 新增批量草案 Store 接口与内存/JSON 实现，第一版只允许保存草案，不执行提交审核、上架或下架。
- 草案创建必须记录操作者、筛选条件、原因、候选快照、阻塞摘要和 `previewOnly` 标志，为后续审批/执行状态机预留字段。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-F 已完成归档表只读检索预览，后台专项可在用户端交易链路稳定后回到 M9-G；当前连续执行指针为 ADM-IA-B-C-E 课程商品发布队列服务端聚合契约与批量草案，CUX-I-B-B-T 高风险动作执行开关、会员待支付订单过期状态机和正式证书签发审核流需另立任务包。
