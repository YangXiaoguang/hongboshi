...
- 当前阶段：`UX-S 售后进度通知与个人中心消息联动`
- 当前状态：`UX-S 售后进度通知与个人中心消息联动` 已完成，售后后台动作会写入账号级站内通知；用户可从顶部消息入口进入个人中心消息 tab，查看售后处理中、已解决、已关闭、退款已受理和退款暂未受理进度，订单详情也会展示同一批售后进度。
- 本轮完成后下一步：执行 `UX-T 退款成功回调与售后工单自动收尾通知`
...
### 最近完成阶段：UX-S 售后进度通知与个人中心消息联动

UX-S 已交付：

- `shared/domain/userNotification.ts`：新增账号级站内消息契约，覆盖售后处理中、已解决、已关闭、退款已受理和退款暂未受理；通知资源固定为 `order_after_sales`，只保留订单号、课程名摘要、工单号、交易流水和退款受理单号。
- `server/modules/users/userNotificationStore.ts` 与 `userNotificationApi.ts`：新增内存/JSON 通知 Store、`GET /api/user-notifications/me` 和 `PATCH /api/user-notifications/me/read`，当前只允许登录用户读取和标记自己的站内消息。
- `server/modules/orders/orderAfterSalesAdminApi.ts`：售后后台动作成功后写入用户通知；退款渠道拒绝受理时保留售后与订单原状态，同时写入“退款申请暂未受理”通知。
- `client/src/features/users/api/httpUserNotificationRepository.ts` 与 `client/src/pages/PersonalCenter.tsx`：个人中心新增“消息” tab，顶部铃铛进入消息中心，支持全部标记已读；订单详情售后服务区展示同一批售后进度。
- `docs/domain-contracts.md` 与 `docs/product-engineering-roadmap.md` 已同步站内消息、隐私最小化边界和“不接真实短信/微信/邮件”的首版范围。

UX-S 验收结果：

- `pnpm check` 已通过。
- 售后通知相关测试已通过：111 个测试文件 / 511 个测试完成，覆盖通知契约、通知 API、前端通知仓储和售后后台动作写通知。
- `pnpm run ci` 已通过：类型检查、111 个测试文件 / 511 个测试和生产构建均完成；Vite 仍保留既有大 chunk 提醒。
- 浏览器已验证 `/me?tab=messages`：登录用户 `138****9019` 后消息中心、未读消息指标、隐私提示和全部标记已读入口正常渲染；从 `/assessment` 顶部消息铃铛可跳转到消息中心，当前移动宽度无横向溢出。

### 用户端待续：UX-T 退款成功回调与售后工单自动收尾通知

业务目标：

在 UX-S 已有售后处理通知基础上，把支付侧 `refund.succeeded` 回调与售后工单最终收尾打通：当真实或模拟退款成功回调抵达时，订单进入 `refunded` 后，相关售后工单和用户站内消息也能显示“退款已完成”的最终状态。

建议实施范围：

- 扩展售后状态或通知类型，新增退款成功/售后自动收尾的 summary-only 契约。
- 支付退款成功回调处理订单状态后，按订单关联售后工单写入最终进度通知，避免用户只看到“退款已受理”。
- 个人中心消息和订单详情展示退款完成通知，但仍不暴露支付 raw payload。
- 增加支付回调、售后 Store、通知 Store 和用户端仓储测试。
- 更新测试、领域契约、路线图和本执行状态。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-F 已完成归档表只读检索预览，后台专项可在用户端交易链路稳定后回到 M9-G；用户端当前连续执行指针为 UX-T 退款成功回调与售后工单自动收尾通知，正式证书签发审核流需另立任务包。
