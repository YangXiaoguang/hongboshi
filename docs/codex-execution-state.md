...
- 当前状态：`ADM-IA-B-C-C 课程商品列表行与筛选组件化` 已完成，课程商品列表页已拆出指标、最近审计、筛选区和列表行组件，主页面收敛为数据加载、权限判断、分页和动作弹窗编排。
- 本轮完成后下一步：执行 `ADM-IA-B-C-D 课程商品发布队列只读分组与批量操作预案`
...
### 后台待续：ADM-IA-B-C-D 课程商品发布队列只读分组与批量操作预案

业务目标：

列表行和筛选区组件化后，可以在不新增高风险批量写动作的前提下，把课程商品中心进一步做成“发布运营队列”。下一步先补只读分组、批量候选预案和发布风险提示，让运营能快速识别待补内容、待审核、待上架、已发布但需复查的商品集合，再为未来批量提交审核、批量上架和二次审批留下清晰边界。

建议实施范围：

- 在 `CourseProductMetrics` 或独立发布队列摘要中增加只读队列分组：待补内容、待提交审核、待审核、审核通过待上架、已上架需复查。
- 基于现有列表数据和内容质量结果生成批量候选预案，只展示候选数量、阻塞原因和风险分布，不调用批量写 API。
- 增加“批量操作暂未开放/需二次审批”的 UI 边界说明，避免运营误以为当前可直接批量发布。
- 如需要新增共享类型，优先放在 `shared/domain`；本阶段默认不新增批量提交审核、批量上架或批量下架真实动作。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-F 已完成归档表只读检索预览，后台专项可在用户端交易链路稳定后回到 M9-G；当前连续执行指针为 ADM-IA-B-C-D 课程商品发布队列只读分组与批量操作预案，CUX-I-B-B-T 高风险动作执行开关、会员待支付订单过期状态机和正式证书签发审核流需另立任务包。
