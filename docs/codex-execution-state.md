...
- 当前阶段：`UX-D 成长空间学习计划承接`
- 当前状态：`UX-D 成长空间学习计划承接` 已完成，`/me/courses` 已把学习计划提升为首屏主线，并按进行中、收藏待学、已完成分组；同路径下一课、收藏转学习计划、测评与咨询辅助区已形成闭环。
- 本轮完成后下一步：执行 `UX-E 课程学习页与章节进度承接`
...
在 UX-C/UX-D 已完成课程详情加入计划和成长空间承接后，继续补齐用户真正进入课程后的学习页。用户点击“继续学习”时，应进入一个围绕章节、学习进度、下一章、同路径下一课和资料占位的课程学习体验，而不是只回到详情页。

实施范围：

- 新增或接入 `/courses/:courseId/learn` 学习页路由，读取课程详情、权益状态和现有课程 engagement 进度。
- 详情页和成长空间中的“继续学习/复习课程/加入学习计划后继续”入口优先进入学习页；未解锁课程仍回到详情页购买或开通会员。
- 学习页首屏展示课程标题、所属学习路径、当前章节、总进度、下一章 CTA 和返回详情/成长空间入口。
- 学习页章节列表支持标记完成，复用 `completeChapter`，不新增服务端状态机。
- 学习页展示资料/练习占位和咨询辅助入口，但不抢占章节学习主线。
- 学完全部章节后展示完成状态、复习入口和同路径下一门课程建议。
- 增加必要的模型或页面测试，覆盖当前章节选择、完成进度、已完成状态和未解锁保护。
- 运行 `pnpm run ci`。
- 更新本文件，提交并推送，建议 commit message：`Add course learning workspace`

验收标准：

- 从课程详情或成长空间点击可学习课程的主 CTA，会进入 `/courses/:courseId/learn`。
- 学习页能清楚展示当前章节、总进度、章节完成动作和下一步。
- 未解锁课程不能绕过购买/会员权限进入学习状态。
- 完成章节后，成长空间和详情页能读取到相同进度。
- 完成所有章节后，学习页出现完成反馈和同路径下一课建议。
- 移动端学习页不会出现标题、按钮、章节列表或进度条溢出。
- CI 通过。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-E 已完成归档后台入口与只读校验，后台专项可继续 M9-F 归档表只读检索预览；当前用户端连续执行指针为 UX-E 课程学习页与章节进度承接。
