...
- 当前状态：`ADM-PRO-H 工作台审核动作接入 + 内容质量问题定位` 已完成，发布审核步骤已接入提交/通过/驳回/撤回审核动作，服务端内容质量阻塞会结构化返回并定位到工作台步骤。
- 本轮完成后下一步：执行 `ADM-PRO-I 工作台上架状态联动 + 商品中心发布队列减负`
...
### 后台待续：ADM-PRO-I 工作台上架状态联动 + 商品中心发布队列减负
...
- 在发布审核步骤接入既有 `updateCourseProductStatus`，仅在审核通过且状态允许时展示上架/下架动作，并保留 `catalog:publish` 权限与审计原因。
- 在工作台发布信息中增加状态流转提示，明确草稿、未提交、待审核、已通过、已上架、已下架、归档之间的可操作边界。
- 商品中心列表保留批量视角，但将单商品编辑链路优先引导到工作台，减少一行内同时出现太多操作按钮。
- ADM-IA-B-C-C 课程商品列表行与筛选组件化继续作为代码减负 backlog，等工作台发布闭环稳定后再执行。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-F 已完成归档表只读检索预览，后台专项可在用户端交易链路稳定后回到 M9-G；当前连续执行指针为 ADM-PRO-I 工作台上架状态联动 + 商品中心发布队列减负，ADM-IA-B-C-C 课程商品列表行与筛选组件化、CUX-I-B-B-T 高风险动作执行开关、会员待支付订单过期状态机和正式证书签发审核流需另立任务包。
