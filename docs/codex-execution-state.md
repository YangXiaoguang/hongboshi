...
- 当前阶段：`UX-Q 用户端售后申请工单与后台交易联动`
- 当前状态：`UX-Q 用户端售后申请工单与后台交易联动` 已完成，用户可在个人中心订单详情对已支付/退款中订单提交售后申请；服务端售后 Store/API 只追加保存申请并校验登录、订单归属和订单状态；后台订单台和交易台可读取售后摘要，交易台会把待处理售后纳入异常提示。
- 本轮完成后下一步：执行 `UX-R 售后工单后台处理动作与退款申请联动`
...
### 最近完成阶段：UX-Q 用户端售后申请工单与后台交易联动

UX-Q 已交付：

- `shared/domain/order.ts`：新增 `OrderAfterSalesCreateRequestSchema`、`OrderAfterSalesRequestSchema`、`OrderAfterSalesSummarySchema`、列表结果和 mutation 结果契约；订单/交易后台详情增加售后申请摘要，交易异常码增加 `after_sales_open`。
- `server/modules/orders/orderAfterSalesStore.ts` 与 `orderAfterSalesApi.ts`：新增独立售后申请 Store/API，支持内存和 JSON 文件持久化；创建申请会校验登录用户订单归属、订单状态、字段长度和重复处理中申请，写入只追加记录，不修改订单退款状态。
- `client/src/pages/PersonalCenter.tsx`：个人中心订单详情抽屉新增售后申请状态展示和表单；已支付/退款中订单可提交无法学习、重复扣款、退款咨询、票据凭证和其他问题。
- `server/modules/orders/orderAdminApi.ts`、`server/modules/transactions/transactionAdminApi.ts` 与后台页面：订单台和交易台详情读取售后申请脱敏摘要；交易台把待处理售后纳入异常提示和处理时间线。
- `docs/domain-contracts.md` 与 `docs/product-engineering-roadmap.md` 已同步售后申请契约、后台联动和“用户端不直接推进退款状态”的边界。

UX-Q 验收结果：

- `pnpm check` 已通过。
- `pnpm run ci` 已通过：类型检查、107 个测试文件 / 494 个测试和生产构建均完成。
- 浏览器已验证 `/me?tab=orders&orderId=order_course_16_u_phone_9019_1779102135962`：登录用户 `138****9019` 后订单详情深链自动打开，已支付订单可提交售后申请，提交后回显工单号、申请类型、`已提交` 状态和“退款不会在用户端直接完成”的边界提示。
- 后台订单/交易售后摘要、交易异常提示和售后时间线已通过 `orderAdminApi`、`transactionAdminApi` 与共享契约测试覆盖。

### 用户端待续：UX-R 售后工单后台处理动作与退款申请联动

业务目标：

在 UX-Q 已有用户端售后申请和后台只读摘要基础上，为运营建立受控处理动作：运营可以认领/处理中/关闭售后申请，并在符合条件时从售后申请联动到交易退款申请，形成“用户诉求 -> 后台处理 -> 退款受理 -> 回调完成”的可追踪链路。

建议实施范围：

- 扩展售后申请 Store/API，新增后台处理动作：标记处理中、关闭/解决、关联交易流水和关联退款受理结果。
- 后台订单台或交易台提供售后申请处理面板，操作必须填写原因并写入审计。
- 对已支付且交易流水安全的订单，允许运营从售后申请触发既有 `request_refund` 交易动作；渠道受理成功后售后申请进入 `linked_to_refund`，订单进入 `refunding`。
- 用户端订单详情显示售后申请处理状态、后台备注摘要和退款受理提示，但仍不得直接改写订单状态。
- 更新测试、领域契约、路线图和本执行状态。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-F 已完成归档表只读检索预览，后台专项可在用户端交易链路稳定后回到 M9-G；用户端当前连续执行指针为 UX-R 售后工单后台处理动作与退款申请联动，正式证书签发审核流需另立任务包。
