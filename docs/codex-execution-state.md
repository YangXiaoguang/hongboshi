...
- 当前状态：`ADM-IA-B-C-F 课程商品发布队列草案审批与漂移预检` 已完成；本轮补齐课程商品运营体验修正，商品内容编辑默认简化、图片上传容错增强，用户端课程详情新增购买决策条与图文详情渐进展开。审批通过仍只确认草案快照有效，不执行提交审核、审核通过或上架。
- 本轮完成后下一步：执行 `ADM-IA-B-C-G 课程商品发布队列执行预案与审计事件设计`
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-F 已完成归档表只读检索预览，后台专项可在用户端交易链路稳定后回到 M9-G；当前连续执行指针为 ADM-IA-B-C-G 课程商品发布队列执行预案与审计事件设计，CUX-I-B-B-T 高风险动作执行开关、会员待支付订单过期状态机和正式证书签发审核流需另立任务包。
