...
- 当前状态：`ADM-PRO-G H5 图片块素材选择增强 + 商品发布前移动预览收口` 已完成，H5 图片块可直接选择已审核素材，发布审核步骤已增加移动成交页预览和发布前预检清单。
- 本轮完成后下一步：执行 `ADM-PRO-H 工作台审核动作接入 + 内容质量问题定位`
...
### 后台待续：ADM-PRO-H 工作台审核动作接入 + 内容质量问题定位
...
- 在发布审核步骤接入既有课程商品审核动作，至少支持提交审核、撤回审核和查看审核状态，不绕过既有 `updateCourseProductReview` API。
- 将内容质量问题定位到对应工作台步骤，预检清单可一键跳转到商品图片、H5 详情或价格权益。
- 对服务端返回的内容质量阻塞/提醒做结构化展示，避免运营只看到一条笼统失败消息。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-F 已完成归档表只读检索预览，后台专项可在用户端交易链路稳定后回到 M9-G；当前连续执行指针为 ADM-PRO-H 工作台审核动作接入 + 内容质量问题定位，ADM-IA-B-C-C 课程商品列表行与筛选组件化、CUX-I-B-B-T 高风险动作执行开关、会员待支付订单过期状态机和正式证书签发审核流需另立任务包。
