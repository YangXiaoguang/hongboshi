...
- 最后更新时间：2026-05-25 Asia/Shanghai
- 当前分支：`main`
- GitHub 仓库：`https://github.com/YangXiaoguang/hongboshi.git`
- 最近已知基线提交：本轮提交后以 Git 历史最新提交为准
- 当前阶段：`ADM-PRO 课程商品详情装修器与成交内容运营升级`
- 当前状态：`ADM-PRO-L 区块排序、复制与模板持久化` 已完成；详情装修器支持 H5 区块上移、下移、复制、删除确认和本地运营模板草案保存/套用/删除，仍保持受控 schema，不开放任意 HTML。发布队列 `ADM-IA-B-C-G` 暂缓，待商品编辑体验稳定后回到批量执行预案。
- 本轮完成后下一步：执行 `ADM-PRO-M 服务端模板库与装修动作审计`
...
- 完成详情装修器区块编排与模板草案持久化：H5 区块支持上移、下移、复制和删除确认；右侧整体面板新增本地运营模板草案保存、套用和删除，保存当前区块结构、文案和样式，套用后仍需通过既有图文内容保存进入服务端 `content_update`。
...
- 模板库需区分个人草案、团队共享模板和系统模板，首版可只开放个人草案与系统模板读取，不影响前台发布内容。
- 为模板保存、套用、删除和区块编排补充装修动作审计预案，明确哪些动作只记录本地草稿提示，哪些动作随 `content_update` 写入课程商品审计。
