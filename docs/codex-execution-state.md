...
- 当前阶段：`CUX-D 全局待支付订单召回`
- 当前状态：`CUX-C 商品信任模块` 已完成，课程详情页新增购买信任区，展示讲师与内容审核、学习反馈、售后/隐私边界和购买前 FAQ；信任信息由 `courseTrust` 纯模型生成，便于后续接真实评价、讲师资质和售后配置。
- 本轮完成后下一步：执行 `CUX-D 全局待支付订单召回`
