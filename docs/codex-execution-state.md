...
- 当前阶段：`UX-H 成长空间学习档案与阶段证明承接`
- 当前状态：`UX-H 成长空间学习档案与阶段证明承接` 已完成，`/me/courses` 已将已完成课程升级为学习档案，承接完成反馈、练习状态、阶段证明预览、待补练习提醒和继续路径入口。
...
- 在 `shared/domain` 建立课程学习记录同步契约，统一描述课程进度、章节完成、练习草稿/完成状态、学习档案摘要、阶段证明预览、同步状态和隐私边界。
- 新增服务端课程学习记录模块，提供 Store 接口与开发期实现，优先支持按当前登录用户读取/写入自己的学习记录，并为 PostgreSQL 持久化预留结构。
- 新增学习记录 API，例如读取当前用户课程学习记录、保存章节进度、保存练习记录、读取学习档案摘要；所有写接口必须校验登录态、课程存在性和课程访问权限。
- 前端课程学习页和成长空间新增 repository 边界，在服务端同步可用时使用 API，同步失败时保持现有本地体验和 `syncStatus` 提示，不丢失用户本地记录。
- 阶段证明保持 `preview/pending` 口径：服务端只输出签发资格、预览字段和待补原因，不生成正式证书编号，不声称已签发。
- 明确同一用户多端记录合并规则，优先保证章节完成单调递增、练习草稿不被空内容覆盖、完成时间稳定可追踪。
- 增加服务端、共享契约、前端 repository 和关键页面测试，覆盖权限、未解锁课程写入保护、旧本地记录兼容、同步失败 fallback、阶段证明预览和空状态。
- 运行 `pnpm run ci`。
- 更新本文件，提交并推送，建议 commit message：`Add course learning record sync`

验收标准：

- 登录用户完成章节或保存练习后，服务端能读取到同一用户的学习记录，刷新页面后仍能恢复进度和练习状态。
- 未登录用户或同步失败时仍可使用现有本地学习体验，并能看到合理的同步状态，不出现空白页或数据丢失。
- 未购买或未具备权益的课程不能通过学习记录 API 被写成已完成。
- `/me/courses` 学习档案可基于服务端记录生成同 UX-H 一致的完成档案、待补练习和阶段证明预览。
- 阶段证明只显示预览/待签发/待补原因，不生成正式证书编号，不产生虚假签发状态。
- 多端或重复提交章节完成时不会回退进度；练习草稿不会被空提交误覆盖。
- CI 通过。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-E 已完成归档后台入口与只读校验，后台专项可继续 M9-F 归档表只读检索预览；当前用户端连续执行指针为 UX-I 课程学习记录服务端同步与阶段证明签发准备。
