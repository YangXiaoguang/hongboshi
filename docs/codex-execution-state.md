...
- 当前阶段：`UX-R 售后工单后台处理动作与退款申请联动`
- 当前状态：`UX-R 售后工单后台处理动作与退款申请联动` 已完成，运营可在订单台和交易台对售后工单执行处理中、解决、关闭和联动退款申请；联动退款复用交易后台 `request_refund` 状态机，渠道受理成功后售后进入 `linked_to_refund` 且订单只推进到 `refunding`，用户端订单详情展示运营备注和退款受理提示。
- 本轮完成后下一步：执行 `UX-S 售后进度通知与个人中心消息联动`
...
### 最近完成阶段：UX-R 售后工单后台处理动作与退款申请联动

UX-R 已交付：

- `shared/domain/order.ts`：新增 `OrderAfterSalesAdminActionRequestSchema`、`OrderAfterSalesAuditEventSchema` 和 `OrderAfterSalesAdminMutationResultSchema`，售后摘要扩展运营备注、关联交易流水和退款受理单号；交易退款申请支持携带 `afterSalesRequestId`。
- `server/modules/orders/orderAfterSalesStore.ts` 与 `orderAfterSalesAdminApi.ts`：售后 Store 支持按 ID 读取、保存、追加审计和读取工单审计；新增 `PATCH /api/orders/admin/after-sales/:requestId/actions`，提供处理中、解决、关闭和联动退款申请动作。
- `server/modules/transactions/transactionAdminApi.ts`：从售后工单联动的 `request_refund` 会校验工单归属和开放状态，允许匹配的售后工单消除 `after_sales_open` 阻塞；渠道拒绝时保留订单和售后原状态。
- `/admin/orders` 与 `/admin/transactions`：订单详情和交易详情中的售后摘要已加入处理面板，运营必须填写原因；联动退款会选择当前可退款支付流水或交易详情当前流水。
- `client/src/pages/PersonalCenter.tsx`：个人中心订单详情展示运营处理备注和退款受理单号提示，但不在用户端直接改写退款完成状态。
- `docs/domain-contracts.md` 与 `docs/product-engineering-roadmap.md` 已同步售后后台动作、双权限边界、审计事件和退款回调仍为最终完成信号的业务约束。

UX-R 验收结果：

- `pnpm check` 已通过。
- 售后后台动作相关测试已通过：108 个测试文件 / 501 个测试完成，覆盖共享契约、后台 API 权限、处理中/解决、联动退款成功、渠道拒绝、交易台阻塞解除和前端仓储解析。
- `pnpm run ci` 已通过：类型检查、108 个测试文件 / 501 个测试和生产构建均完成；Vite 仍保留既有大 chunk 提醒。
- 浏览器已验证 `/me?tab=orders&orderId=order_course_16_u_phone_9019_1779102135962`：登录用户 `138****9019` 后订单详情深链自动打开，售后服务区可见且页面无横向溢出；`/admin/orders` 在普通会员身份下正确显示后台权限提示。后台售后处理动作因本地 UI 无运营账号入口，已通过 API 与仓储测试覆盖权限和状态流转。

### 用户端待续：UX-S 售后进度通知与个人中心消息联动

业务目标：

在 UX-R 已有后台处理动作和退款受理联动基础上，为用户建立可见、不过度打扰的售后进度通知：用户不必反复进入订单详情，也能在个人中心看到售后工单处理变化、退款受理和后续待办。

建议实施范围：

- 建立账号级消息/通知共享契约，首版只做站内消息，不接真实短信、微信或邮件。
- 后台售后动作成功后写入用户可读的售后进度通知，包括处理中、已解决、已关闭、退款已受理和退款渠道拒绝。
- 个人中心新增消息或服务通知入口，并在订单详情中展示同一批售后进度事件。
- 通知内容保持隐私最小化，只展示订单号、课程名、状态、时间和运营备注摘要，不展示支付敏感原文。
- 更新测试、领域契约、路线图和本执行状态。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-F 已完成归档表只读检索预览，后台专项可在用户端交易链路稳定后回到 M9-G；用户端当前连续执行指针为 UX-S 售后进度通知与个人中心消息联动，正式证书签发审核流需另立任务包。
