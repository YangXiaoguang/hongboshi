...
- 当前阶段：`UX-E 课程学习页与章节进度承接`
- 当前状态：`UX-E 课程学习页与章节进度承接` 已完成，新增 `/courses/:courseId/learn` 学习页，课程详情和成长空间的继续学习入口已进入学习页；学习页可展示当前章节、总进度、章节完成动作、资料/练习占位、咨询支持和同路径下一课建议，未解锁课程不会绕过权益边界。
- 本轮完成后下一步：执行 `UX-F 课程资料与练习记录闭环`
...
在 UX-E 已完成课程学习页和章节进度承接后，把学习页右侧“资料与练习”从静态占位推进到可维护的学习辅助闭环。第一版仍以本地状态和现有课程内容契约为主，重点是稳定章节资料展示、练习记录保存、完成状态反馈和后续接真实文件/服务端学习记录的扩展边界。

实施范围：

- 新增课程学习资料/练习记录模型，例如按 `courseId + chapterId` 组织讲义、练习草稿、练习完成状态、更新时间和本地来源。
- 在课程学习页中把当前章节的“章节讲义、练习记录、素材状态”改为真实可交互区：可查看资料摘要、填写练习记录、保存草稿、标记练习完成。
- 练习记录第一版使用本地持久化 repository，结构上预留服务端同步字段，不和课程章节完成状态混写。
- 当前章节切换后，资料与练习面板应自动读取对应章节记录；完成章节时可提示是否补齐练习，但不强制阻塞章节完成。
- 课程完成态应展示练习完成摘要，帮助用户知道哪些章节已经有复盘记录。
- 增加必要的模型、repository 和页面辅助测试，覆盖记录保存、章节切换、练习完成统计、空状态和历史数据兼容。
- 运行 `pnpm run ci`。
- 更新本文件，提交并推送，建议 commit message：`Add course practice workspace`

验收标准：

- 用户在 `/courses/:courseId/learn` 可为当前章节保存练习记录，刷新后仍能读取。
- 切换或完成章节后，资料与练习面板展示对应章节内容，不串章。
- 练习完成状态与章节完成状态各自独立，但课程完成反馈能展示练习摘要。
- 未解锁课程仍不能访问资料与练习交互区。
- 本地数据结构能兼容历史空数据和后续服务端同步字段。
- 移动端资料区、输入区和按钮不溢出。
- CI 通过。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-E 已完成归档后台入口与只读校验，后台专项可继续 M9-F 归档表只读检索预览；当前用户端连续执行指针为 UX-F 课程资料与练习记录闭环。
