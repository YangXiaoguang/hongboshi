...
- 当前状态：`ADM-PRO-I 工作台上架状态联动 + 商品中心发布队列减负` 已完成，发布审核步骤已接入上架/下架动作和发布状态流，商品中心列表行已收敛为工作台/发布管理主入口。
...
商品编辑、审核和上架主链路已经进入工作台，课程商品列表可以进一步收敛为队列视图。下一步要把列表行、筛选区、指标区和审计摘要拆成稳定组件，降低 `CourseProducts.tsx` 的维护压力，并为后续批量发布队列、批量筛选和更多运营指标预留边界。

建议实施范围：

- 拆出课程商品列表行组件，保留状态展示、内容质量 badge、工作台/发布管理入口和只读 fallback。
- 拆出课程商品筛选区与指标区，保持 URL 查询同步和分页行为不变。
- 拆出最近审计摘要组件或模型，避免商品列表主文件继续承载审计展示细节。
- 不新增新的写动作，不改变服务端权限与审计契约；本阶段以可维护性和 UI 减负为主。
