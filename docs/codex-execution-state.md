...
- 当前阶段：`UX-P 用户端订单详情与售后服务入口`
- 当前状态：`UX-P 用户端订单详情与售后服务入口` 已完成，个人中心订单列表和优惠券关联订单可打开统一订单详情抽屉，展示金额、优惠、支付、权益交付、时间线和售后说明；待支付订单支持继续支付与取消，已支付订单支持查看课程和成长空间。
- 本轮完成后下一步：执行 `UX-Q 用户端售后申请工单与后台交易联动`
...
### 最近完成阶段：UX-P 用户端订单详情与售后服务入口

UX-P 已交付：

- `client/src/features/courses/model/personalOrderDetail.ts`：新增用户端订单详情纯模型，统一生成金额明细、订单时间线和售后服务说明，保持退款只进入受控说明，不在用户端直接改变状态。
- `client/src/features/courses/model/personalOrderDetail.test.ts`：覆盖优惠金额行、支付/交付时间线和售后说明边界。
- `client/src/pages/PersonalCenter.tsx`：新增订单详情抽屉，订单列表“订单详情”和优惠页“查看订单”复用同一入口；详情展示订单号、状态、商品、金额、支付渠道、优惠券、权益交付、时间线和售后说明。
- 待支付订单详情提供“继续支付”和“取消订单”；已支付订单详情提供“查看课程”和“查看成长空间”，退款/售后仅作为人工处理说明，不直接制造退款成功态。
- `docs/domain-contracts.md` 与 `docs/product-engineering-roadmap.md` 已同步用户端订单详情复用 `Order` 与 `couponApplication` 的边界。

UX-P 验收结果：

- `pnpm check` 已通过。
- `pnpm run ci` 已通过：类型检查、106 个测试文件 / 488 个测试和生产构建均完成。
- 浏览器已验证 `/me?tab=orders` 可打开订单详情抽屉，`/me?tab=orders&orderId=...` 深链可直接恢复同一详情；金额明细、优惠券、权益交付、订单时间线、售后说明、查看课程和查看成长空间入口均可见，移动宽度无横向溢出。

### 用户端待续：UX-Q 用户端售后申请工单与后台交易联动

业务目标：

在 UX-P 的订单详情基础上，建立受控的用户端售后申请入口：用户可以对已支付订单提交“无法学习/重复扣款/退款咨询”等售后诉求，系统生成后台可追踪工单，由运营/交易退款台处理，避免用户端直接改写退款状态。

建议实施范围：

- 新增用户端售后申请共享契约，描述申请类型、订单 ID、说明、联系方式、状态和创建时间。
- 服务端建立售后申请 Store/API，校验登录、订单归属、订单状态和敏感字段长度，写入只追加申请记录。
- 个人中心订单详情中为已支付/退款中订单提供售后申请表单入口，提交后展示工单状态和人工处理提示。
- 后台交易退款台或订单台读取售后申请摘要，并与现有交易异常工单/退款申请动作建立关联。
- 用户端仍不得直接推进 `refunding/refunded`，退款状态只能由后台受控动作和支付回调驱动。
- 更新测试、领域契约、路线图和本执行状态。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-F 已完成归档表只读检索预览，后台专项可在用户端交易链路稳定后回到 M9-G；用户端当前连续执行指针为 UX-Q 用户端售后申请工单与后台交易联动，正式证书签发审核流需另立任务包。
