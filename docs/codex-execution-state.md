...
- 当前阶段：`UX-F 课程资料与练习记录闭环`
- 当前状态：`UX-F 课程资料与练习记录闭环` 已完成，课程学习页右侧资料与练习区已从静态占位升级为章节级学习工作台；练习草稿、练习完成状态、本地持久化记录和完成摘要均已独立于章节完成状态，未解锁课程仍不能访问资料与练习交互。
- 本轮完成后下一步：执行 `UX-G 课程完成反馈与阶段证书准备`
...
在 UX-F 已完成章节资料与练习记录闭环后，为用户完成课程时提供更明确的阶段反馈：让用户知道自己完成了什么、沉淀了哪些练习、下一步适合继续哪条学习路径，并为后续真实证书、学习报告和服务端学习档案预留稳定边界。第一版仍以本地学习记录和现有课程/路径模型为主，不做真实证书签发和对外分享。

实施范围：

- 新增课程完成反馈派生模型，例如基于课程、学习会话、练习摘要和课程路径输出完成标题、完成率、练习沉淀、建议下一步和证书预备数据。
- 在课程学习页完成态区域升级“完成反馈”：展示课程完成总结、章节完成、练习完成、学习时长、适合继续的下一门课程和回到成长空间入口。
- 增加阶段证书预览卡片，展示课程名、完成时间、学习路径、章节数和练习完成数；明确为“阶段证明预览/待正式签发”，不制造真实证书编号。
- 完成反馈应兼容未做练习、只完成部分练习、无同路径下一课和未登录/本地记录等状态。
- 证书预备模型需预留 `source`、`issuedAt`、`certificateId`、`syncStatus` 等字段，为后续服务端签发和学习档案同步留接口。
- 增加必要的模型和页面测试，覆盖完成反馈派生、证书预览字段、下一课推荐、空练习状态和完成态渲染。
- 运行 `pnpm run ci`。
- 更新本文件，提交并推送，建议 commit message：`Add course completion feedback`

验收标准：

- 完成课程后，`/courses/:courseId/learn` 展示清晰的课程完成反馈、练习沉淀摘要和下一步建议。
- 证书预览卡片展示稳定字段，但不声称已正式签发真实证书。
- 未完成课程不提前展示完成反馈或证书预览。
- 无练习记录、无下一课和本地数据为空时页面仍有可信空状态。
- 未解锁课程仍不能访问完成反馈、资料与练习交互或证书预览。
- 移动端完成反馈区、证书预览和按钮不溢出。
- CI 通过。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-E 已完成归档后台入口与只读校验，后台专项可继续 M9-F 归档表只读检索预览；当前用户端连续执行指针为 UX-G 课程完成反馈与阶段证书准备。
