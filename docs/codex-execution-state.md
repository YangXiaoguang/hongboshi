...
- 当前状态：`ADM-IA-B-C-E 课程商品发布队列服务端聚合契约与批量草案` 已完成，发布队列已从前端临时聚合升级为服务端契约/API，后台可保存只读批量草案快照但不会执行提交审核、审核通过或上架。
- 本轮完成后下一步：执行 `ADM-IA-B-C-F 课程商品发布队列草案审批与漂移预检`
...
### 后台待续：ADM-IA-B-C-F 课程商品发布队列草案审批与漂移预检
...
- 扩展发布队列草案状态机：`draft -> pending_approval -> approved/rejected/canceled`，增加审批人、审批原因、审批前后摘要和不可自审规则。
- 新增草案漂移预检服务：对比创建时 `candidateSnapshot` 与当前服务端发布队列，输出消失候选、新增候选、状态变化、内容质量变化、风险变化和是否需要重建。
- 后台发布队列面板或独立弹窗展示草案详情、候选快照、漂移预检和审批动作；执行按钮继续隐藏或禁用。
- 为未来执行设计审计事件结构和落点，但本阶段仍不写课程商品 `status_update/review_update` 审计，不调用批量状态变更。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-F 已完成归档表只读检索预览，后台专项可在用户端交易链路稳定后回到 M9-G；当前连续执行指针为 ADM-IA-B-C-F 课程商品发布队列草案审批与漂移预检，CUX-I-B-B-T 高风险动作执行开关、会员待支付订单过期状态机和正式证书签发审核流需另立任务包。
