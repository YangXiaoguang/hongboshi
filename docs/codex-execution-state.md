...
- 当前阶段：`UX-C 课程详情转化与学习计划入口`
- 当前状态：`UX-C 课程详情转化与学习计划入口` 已完成，课程详情页已展示所属学习路径、路径位置、下一门建议课程、权益状态 CTA、学习计划入口和双层相关推荐；免费课加入学习计划后可切换为继续学习，咨询陪伴入口已进入咨询服务页。
- 本轮完成后下一步：执行 `UX-D 成长空间学习计划承接`
...
在 UX-C 已让课程详情页支持“加入学习计划”后，继续优化 `/me/courses` 成长空间，让用户能清楚看到自己正在学什么、下一门课是什么、收藏课程如何转为学习计划，以及测评/咨询如何作为辅助支持进入个人成长闭环。

实施范围：

- 深读 `client/src/pages/MyCourses.tsx` 和课程 engagement 模型，梳理当前成长空间的信息层级。
- 抽出学习计划派生模型，按进行中课程、已收藏课程、已完成课程、同路径下一课组织数据，复用 `coursePath` 和 `courseEngagement`，不新增重复状态。
- 重构 `/me/courses` 首屏，把“学习计划/继续学习/下一步建议”提升为主线，测评报告、咨询预约和会员权益作为辅助区块。
- 为收藏课程提供清晰的“加入学习计划”入口；已开始课程显示进度、最近学习状态和继续学习 CTA。
- 在成长空间中展示所属学习路径与同路径下一课，和 UX-C 课程详情页形成闭环。
- 保持未登录、无学习记录、无收藏、无测评结果等空状态专业可用。
- 增加必要的模型或页面测试，覆盖学习计划分组、路径下一课、空状态和收藏转学习计划。
- 运行 `pnpm run ci`。
- 更新本文件，提交并推送，建议 commit message：`Improve learning plan workspace`

验收标准：

- `/me/courses` 第一屏能清楚回答“我正在学什么、下一步学什么、为什么继续这条路径”。
- 从课程详情加入学习计划后，成长空间能看到对应课程和进度状态。
- 收藏课程和进行中课程不会混在一起，用户能把收藏转为学习计划。
- 测评和咨询仍作为成长辅助入口，不抢占课程学习主线。
- 未登录和空数据状态不会出现断裂、旧文案或不可操作入口。
- 移动端成长空间首屏不会出现标题、按钮或课程卡片溢出。
- CI 通过。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-E 已完成归档后台入口与只读校验，后台专项可继续 M9-F 归档表只读检索预览；当前用户端连续执行指针为 UX-D 成长空间学习计划承接。
