...
- 当前阶段：`UX-B 课程路径与推荐转化体验`
- 当前状态：`UX-B 课程路径与推荐转化体验` 已完成，首页和 `/courses` 已加入可交互课程路径，路径选择会同步课程分类、排序和课程发现标题；`/courses` 增加热门课程、免费入门和会员可学的快速开始区，课程、测评和咨询的转化优先级更清楚。
- 本轮完成后下一步：执行 `UX-C 课程详情转化与学习计划入口`
...
在 UX-B 已把用户从困扰引导到课程路径后，继续优化课程详情页的转化和学习计划入口。用户进入课程详情时，应能清楚看到这门课属于哪类成长路径、为什么适合自己、如何开始学习、购买/会员/免费权益是什么，以及学完后如何进入下一门课或需要咨询支持。

实施范围：

- 在 `CourseDetail` 中接入课程路径识别，展示“所属学习路径”和同路径下一步课程。
- 优化课程详情首屏转化区，明确免费、已购、会员可学、需购买、需会员等不同权益状态下的主按钮文案和次级动作。
- 增加“加入学习计划”或等价学习计划入口，复用现有课程 engagement 状态，不引入新的服务端状态机。
- 将课程详情里的相关课程从单纯同类推荐升级为“继续这条路径”与“你可能也需要”的双层结构。
- 保持咨询预约为深度支持入口，放在课程详情后段或侧边辅助位置，不抢占课程主购买/学习 CTA。
- 增加必要的模型或组件测试，覆盖路径识别、权益 CTA 文案和同路径推荐。
- 运行 `pnpm run ci`。
- 更新本文件，提交并推送，建议 commit message：`Improve course detail conversion`

验收标准：

- 用户从路径课程进入详情后，能看到课程所属路径和下一步课程。
- 不同课程权益状态的主 CTA 清楚，免费/已购/会员/待购买不会混淆。
- 学习计划入口复用现有本地/服务端课程参与度边界，不绕开登录和购买权限。
- 相关课程能区分同路径延续和普通相关推荐。
- 移动端详情首屏不会出现标题、按钮、价格或 CTA 溢出。
- CI 通过。
