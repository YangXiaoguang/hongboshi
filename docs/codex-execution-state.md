...
- 当前阶段：`M7 咨询运营增强`
- 当前状态：`M7-J 咨询预约改期入口与服务前准备体验` 已完成；`/me?tab=orders` 的已预约咨询卡片可显示服务前准备、咨询师摘要和同咨询师可改期时段，用户可在个人中心直接确认改期并由服务端预约状态机释放原时段、锁定新时段。课程商品 `ADM-PRO-T-A` 暂缓，待本轮咨询运营体验稳定后继续。
- 本轮完成后下一步：执行 `M7-K 咨询预约取消退款用户端承接与服务记录回看`
...
- 完成咨询预约待支付召回与个人中心承接体验：`/consulting` 会提示当前账号已有待支付咨询预约，并支持继续支付或取消；新预约生成后可直接确认支付并进入个人中心；`/me` 订单页将咨询预约从课程/会员订单中独立呈现，显示支付保留时间、咨询金额、状态和继续支付/取消动作，避免咨询订单被误导到课程页面。
- 完成咨询预约改期入口与服务前准备体验：`/me?tab=orders` 已预约咨询卡片会展示服务前准备、咨询师摘要和改期入口；改期面板读取当前咨询师可用时段，用户确认后调用服务端 `reschedule` 状态机更新预约时段，卡片时间即时刷新。
