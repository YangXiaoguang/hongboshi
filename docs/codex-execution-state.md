...
- 当前阶段：`UX-G 课程完成反馈与阶段证书准备`
- 当前状态：`UX-G 课程完成反馈与阶段证书准备` 已完成，课程完成后会展示完成反馈、练习沉淀摘要、同路径下一步建议和阶段证明预览；证书预览明确为待正式签发，不生成虚假证书编号，并为后续服务端学习档案/证书签发预留稳定字段。
- 本轮完成后下一步：执行 `UX-H 成长空间学习档案与阶段证明承接`
...
### UX-H: 成长空间学习档案与阶段证明承接

业务目标：

在 UX-G 已完成课程完成反馈和阶段证明预览后，把这些完成结果承接到成长空间，让 `/me/courses` 不只是“课程列表”，而是用户长期学习档案的入口。第一版仍基于本地进度和练习记录，不做真实证书签发、不做对外分享、不上传学习报告。

实施范围：

- 新增成长空间学习档案派生模型，例如从课程列表、学习计划、课程进度、练习摘要和完成反馈中提取已完成课程档案、阶段证明预览和路径下一步。
- `/me/courses` 的已完成课程区升级为“学习档案”视图，展示完成时间、章节完成、练习完成、阶段证明预览状态和复习/继续路径入口。
- 成长空间首屏或侧栏增加完成档案摘要，例如已完成课程数、已生成预览数、练习沉淀数和待补练习提醒，但不抢占“本次继续”主线。
- 已完成课程可进入 `/courses/:courseId/learn` 查看完成反馈；未完成课程不展示阶段证明预览。
- 无已完成课程、无练习记录、无同路径下一课时要有可信空状态。
- 复用 UX-G 的 `createCourseCompletionFeedback` 口径，不在页面中重复实现证书字段与完成判断。
- 增加必要的模型和页面辅助测试，覆盖完成档案列表、证书预览状态、待补练习提示、未完成过滤和空状态。
- 运行 `pnpm run ci`。
- 更新本文件，提交并推送，建议 commit message：`Add learning archive workspace`

验收标准：

- `/me/courses` 能看到已完成课程的学习档案摘要、阶段证明预览状态和复习入口。
- 未完成课程不会出现在阶段证明预览列表中，也不会显示已完成口径。
- 已完成但练习未补齐的课程能展示待补练习提示。
- 从成长空间点击已完成课程可进入学习页查看 UX-G 完成反馈。
- 空状态、无下一课和本地记录为空时页面不报错。
- 移动端学习档案区、阶段证明状态和按钮不溢出。
- CI 通过。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-E 已完成归档后台入口与只读校验，后台专项可继续 M9-F 归档表只读检索预览；当前用户端连续执行指针为 UX-H 成长空间学习档案与阶段证明承接。
