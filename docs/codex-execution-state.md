...
- 当前阶段：`UX-O 结算内领券与订单详情深链`
- 当前状态：`UX-O 结算内领券与订单详情深链` 已完成，结算抽屉可展示未领取但适用的课程券，支持就地领取并自动选中，支付成功态和个人中心优惠页可按订单 ID 跳转并定位订单。
- 本轮完成后下一步：执行 `UX-P 用户端订单详情与售后服务入口`
...
### 最近完成阶段：UX-O 结算内领券与订单详情深链

UX-O 已交付：

- `client/src/features/courses/model/courseCouponBag.ts`：账号券包模型新增 `claimable` 选项，结算时会把当前课程未领取但仍有效的课程券作为可领取券卡返回，并避免把未领取券自动选中。
- `client/src/features/courses/hooks/useUserPreference.ts`：偏好 hook 新增结算内领券动作、领取中状态和未登录登录引导，领取成功后返回最新账号券包。
- `client/src/components/CourseCheckoutDrawer.tsx`：账号券包区支持“领取并使用”，领取中禁用当前券卡；支付成功态展示本单用券信息，并提供“查看订单”入口。
- `client/src/pages/Courses.tsx`、`CourseDetail.tsx`、`MyCourses.tsx`：课程中心、课程详情和成长空间均可在结算抽屉内领取课程券，领取成功后自动选中该 claim，并把支付成功后的订单入口跳到个人中心订单深链。
- `client/src/pages/PersonalCenter.tsx`：支持 `/me?tab=orders&orderId=...`，进入订单 tab 后滚动定位并高亮目标订单；优惠页“查看订单”会定位到已使用券关联的订单。
- `client/src/features/courses/model/courseCouponBag.test.ts`：补充未领取券在结算中显示为 `claimable` 且不会自动默认选中的模型测试。
- `docs/domain-contracts.md` 与 `docs/product-engineering-roadmap.md` 已同步结算内领券、订单深链和前端金额边界。

UX-O 验收结果：

- `pnpm check` 已通过。
- 浏览器验证已通过：登录用户打开 `/courses/16?checkout=course`，结算抽屉展示未领取的“新人专享减20”券卡，点击“领取并使用”后自动切换为“已关联/已选中”，未领取券不会误显示为已关联。
- 浏览器验证已通过：领取后创建并支付课程订单，支付成功态展示订单号、本单用券说明和“查看订单”，点击后进入 `/me?tab=orders&orderId=...` 并定位到对应订单，无横向溢出。
- 浏览器验证已通过：进入 `/me?tab=coupons` 可看到已使用券的关联订单、使用时间和支付渠道，点击“查看订单”同样跳转到订单深链，无横向溢出。
- 完整 `pnpm run ci` 已通过：类型检查、105 个测试文件 / 485 个测试和生产构建均完成；构建保留既有 Vite 大 chunk 提醒。

### 用户端待续：UX-P 用户端订单详情与售后服务入口

业务目标：

把“下单后查信息”和“遇到问题找服务”做成明确的用户端订单工作台：用户可以从结算成功、个人中心订单、优惠券关联订单进入同一套订单详情，清楚看到商品、金额、优惠、支付状态、权益交付、售后边界和下一步动作。

建议实施范围：

- 在个人中心订单列表增加订单详情抽屉或详情页，展示订单状态、商品明细、金额明细、支付渠道、优惠券应用、权益交付和时间线。
- 待支付订单详情提供继续支付、取消订单；已支付订单详情提供查看课程、查看成长空间和售后说明。
- 已使用优惠券的“查看订单”进入同一订单详情，而不是只定位列表。
- 将售后/退款入口先做成受控说明和客服/后台处理入口，不在用户端直接制造退款成功态。
- 复用现有 `Order`、`couponApplication`、课程权益和个人中心订单数据，不新增前端自行计算金额口径。
- 更新测试、领域契约、路线图和本执行状态。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-F 已完成归档表只读检索预览，后台专项可在用户端交易链路稳定后回到 M9-G；用户端当前连续执行指针为 UX-P 用户端订单详情与售后服务入口，正式证书签发审核流需另立任务包。
