...
- 当前阶段：`UX-I 课程学习记录服务端同步与阶段证明签发准备`
- 当前状态：`TRX-C 课程订单状态与支付结果服务端化` 已完成，课程详情页购买流程已从前端直接解锁升级为服务端创建订单、待支付、模拟支付结果、取消、支付成功后交付权益的状态流。
- 本轮完成后下一步：执行 `UX-I 课程学习记录服务端同步与阶段证明签发准备`
...
- 在 `shared/domain` 补充课程学习记录同步契约，覆盖章节进度、练习记录、课程完成反馈、学习档案摘要和阶段证明预览签发准备字段。
- 建立服务端学习记录 Store，开发期优先支持内存与 JSON 文件持久化，并预留 PostgreSQL Store/迁移边界；写入必须绑定登录用户与已解锁课程权益。
- 新增用户端学习记录 API：读取课程学习档案、同步章节完成状态、保存练习记录、提交课程完成反馈、生成阶段证明预览；服务端统一校验课程权益、课程存在性和记录归属。
- 前端 `useCourseEngagement`、`useCoursePractice`、学习页和成长空间从本地优先逻辑升级为“登录态服务端同步 + 本地 fallback”，保留离线/开发期可用性，不破坏已有学习体验。
- 阶段证明仍只生成预览状态，字段中明确 `source`、`syncStatus`、`certificateId`、`issuedAt`、`issuerStatus`，为后续正式签发审核流留稳定接口。
- 增加共享契约、服务端 API/Store、前端 repository/hook、学习页和成长空间承接测试，覆盖未登录、未购买、跨用户访问、重复同步、完成反馈、预览证书和本地 fallback。
- 更新 README、`docs/domain-contracts.md`、`docs/database-schema.md`、`docs/product-engineering-roadmap.md` 和本文件。
- 运行 `pnpm run ci`。
- 提交并推送，建议 commit message：`Add course learning record sync`

验收标准：

- 登录且已购买课程的用户能在学习页同步章节进度和练习记录，刷新页面或重新进入后记录仍可读取。
- 未登录或未获得课程权益的用户不能写入该课程学习记录，服务端返回明确错误，前端展示温和提示。
- 完成课程后，成长空间能读取服务端学习档案摘要，并展示完成时间、练习沉淀、下一步建议和阶段证明预览状态。
- 阶段证明只进入预览/待签发准备，不生成正式证书编号，不允许跨用户查看。
- 本地 fallback 不覆盖服务端真相源，网络失败时不导致已有服务端记录丢失。
- CI 通过。
...
- 交易操作 Store 已独立落表；统一审计中心第一版已先做只读聚合，M9-E 已完成归档后台入口与只读校验，后台专项可继续 M9-F 归档表只读检索预览；当前用户端连续执行指针为 UX-I 学习记录服务端同步与阶段证明签发准备。
