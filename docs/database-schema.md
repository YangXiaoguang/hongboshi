...
- `server/db/migrations/0023_course_product_create_audit.sql`：扩展课程商品审计动作约束，允许记录手动新增课程商品 `product_create`。
...
`server/modules/catalog/postgresCourseProductContentStore.ts` 已实现 `course_product_contents` 的读取、保存和清空能力。表内以 `JSONB` 保存摘要、`summaryRichText` 白名单摘要富文本、适合人群、章节、素材占位和 `sales_assets` 成交图文素材；`summaryRichText` 仅支持段落、要点和重点样式，不保存任意 HTML；`sales_assets` 现在同时承载受控 `richTextBlocks` H5 内容块，支持标题、正文、图片、要点、FAQ、讲师介绍和购买须知。章节素材绑定仍作为内容 JSONB 的一部分保存，后续如需素材报表、批量治理或跨课程复用，再拆出素材专表与关联表。当配置 `DATABASE_URL`，且 `HONGBOSHI_COURSE_PRODUCT_CONTENT_STORE=postgres` 时，课程详情内容会写入 PostgreSQL。内容更新会写入课程商品审计事件，并把需要复审的商品回退到未提交审核；若请求携带 `sourceTemplate`，模板来源只进入本次 `content_update` 审计 after。
