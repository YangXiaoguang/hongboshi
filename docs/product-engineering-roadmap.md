...
- 完成课程退款后的权益回收和访问边界：单课订单进入 `refunded` 后，仅在没有其他有效已支付同课订单时回收 `ownedCourseIds`；课程中心、详情页和成长空间通过权益状态自动回到购买入口，个人中心已退款订单展示权益已停止与重新购买。会员退款暂保持保守处理，后续需补齐会员来源订单字段后再安全回收。
- 为筛选、分页、收藏、价格展示补单元测试。
