...
- 完成会员商品运营后台首个切片：新增 `membership_product:read/manage` 权限、会员商品后台契约、内存/JSON Store、后台 API 和 `/admin/memberships`，运营可维护会员商品文案、套餐价格、套餐暂停/恢复，并查看审计记录；用户端读取服务端会员商品快照留到下一步。
- 为筛选、分页、收藏、价格展示补单元测试。
...
- 已接入 `/admin/memberships` 会员商品后台，支持会员商品配置、套餐价格、套餐状态生命周期、资源级权限和操作审计，开发期可写入 JSON Store。
