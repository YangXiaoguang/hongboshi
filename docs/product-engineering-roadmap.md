...
- 已接入 `/admin/courses` 课程商品列表，课程 seed 可映射为运营商品契约，并支持搜索、状态、审核流、分类、排序、分页、基础信息编辑、内容质量校验、资源级权限、上下架、改价、审计记录、JSON/PostgreSQL 持久化和前台发布联动；详情内容编辑、成交图文、章节/素材占位、素材上传和合规处理已迁入 `/admin/courses/:courseId` 详情页；当前列表默认只加载商品列表和内容质量，素材治理、学习资料报表、批量任务、队列观测和高风险预案已迁入 `/admin/course-assets/governance` 独立工作区，素材治理模型、治理主面板和页面入口已从课程商品主文件拆出。
