...
- 已完成课程素材资产登记、真实文件上传、受控读取与学习页资料绑定首个切片：新增课程素材资产共享契约、开发期 JSON Store、本地文件存储 adapter、后台素材读取/URL 登记/文件上传/合规处理 API 和 `/admin/courses` 素材资产库区；已通过图片可作为课程详情展示素材，已通过且开启下载的章节资料、练习表、音频和视频可绑定到章节素材占位，学习页只对已解锁用户展示受控下载入口。
- 完成课程详情、章节、学习进度、收藏列表。
