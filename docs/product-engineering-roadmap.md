...
- 后续接入真实支付/退款渠道、账期/手续费规则、结算批次、对账异常处理动作和运营端风险复核。

验收标准：

- 当前阶段：用户可以完成从测评推荐或首页进入咨询预约，咨询师可以进入工作台处理履约状态。
- 下一阶段：真实支付/退款渠道适配、账期/手续费规则、结算批次、对账异常处理动作和风险复核可串联。
...
- 已接入 `/admin/finance` 财务只读台，聚合收入、退款、净收款、退款中金额、异常金额、渠道/业务类型分布和财务明细，财务口径由服务端统一计算，并支持 CSV 导出生成时间、筛选条件、汇总金额、口径版本和预留账务字段。
- 后续继续建立账期、手续费、结算批次、真实渠道适配、咨询师增强、测评和风险后台。
- 增加运营位、优惠券、会员配置、数据看板。
