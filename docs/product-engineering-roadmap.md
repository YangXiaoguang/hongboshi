...
- 完成统一审计 Store 架构方案、归档事件契约和 `audit_center_archived_events` 只追加表草案，明确先归档回填、不替换业务 Store 真相源。
- 后续接入重复排班模板、真实支付/退款渠道、结算批次、渠道结算单、风险通知协作、审计归档任务和对账异常处理动作。

验收标准：

- 当前阶段：用户可以完成从测评推荐或首页进入咨询预约，咨询师可以进入工作台处理履约状态。
- 下一阶段：真实支付/退款渠道适配、结算批次、渠道结算单、对账异常处理动作和审计归档任务可串联。
...
- 已接入 `/admin/audit` 审计中心，按模块、动作、操作者、资源关键词和日期范围只读聚合后台操作审计，并支持 CSV 导出和单条事件详情定位；统一审计 Store 已完成归档方案和表草案。
- 后续继续建立结算批次、真实渠道适配、通知协作、审计归档任务和测评后台。
- 增加运营位、优惠券、会员配置、数据看板。
