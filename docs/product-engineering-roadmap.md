...
- 已完成课程详情成交素材后台化首个切片：`CourseProductDetailContentSchema` 新增 `merchandising` 成交图文内容，后台 `/admin/courses` 可维护详情成交标题、副标题、主视觉、卖点和图文资产；前台课程详情优先展示后台维护的主视觉、卖点和成交图文资产，PostgreSQL `course_product_contents.sales_assets` 保存同一份内容。
- 已完成课程商品摘要轻量富文本持久化与编辑减负：`CourseProductDetailContentSchema` 新增 `summaryRichText` 白名单结构，后台默认图文快编可维护段落、要点和重点样式，前台课程详情按安全组件渲染；工作台套用快速模板或服务端模板后，内容保存审计可记录模板来源；摘要编辑器可按标题、适合人群和卖点生成草稿建议，但不会自动覆盖人工内容。
- 已完成课程详情模板共享审核服务端契约：个人模板可申请团队共享，具备审核权限的账号可通过或驳回；通过会生成独立团队模板快照，驳回会退回私有状态，模板申请、通过、驳回均写入模板动作审计。
- 已完成课程详情模板共享审核前端接入：后台模板库可筛选待审核共享申请，展示申请人、申请时间和最近审计，具备审核权限的管理员可填写审核意见并通过或驳回，处理后刷新团队模板快照。
- 已完成课程素材资产登记、真实文件上传、受控读取、学习页资料绑定和正式存储准备切片：新增课程素材资产共享契约、开发期 JSON Store、本地文件存储 adapter、对象存储 adapter 接口、PostgreSQL 素材 Store、回填 dry-run/受控写入 service、素材治理只读 service、单素材治理动作 service、治理历史/批量草稿预览 service、批量治理任务草案/审批/执行只读预案/受控执行状态机/执行历史筛选 service、批量治理任务 PostgreSQL Store、执行 job 契约、最小队列接口、执行锁、失败安全重试边界、队列观测 service、学习资料运营报表和高风险批量动作只读预案，后台素材读取/URL 登记/文件上传/合规处理 API、`/admin/courses` 素材资产库区和 `/admin/course-assets/governance` 独立治理工作区；对象存储 provider 已支持 `local/s3/oss/cos` 配置边界、短期签名读取 URL、bucket/region/公开域名/签名密钥配置和 provider 级测试；素材治理 API 可识别未引用、重复 hash、待审/驳回、下载关闭、软删候选和引用来源，治理面板可记录处理、标记重复主素材、软删除确认、筛选治理历史、查看批量处理草稿、保存待审批批量草案、审批/驳回、为已审批任务生成只读执行预案并在确认后执行记录处理，也可按审批/执行状态、操作者、问题、动作和日期筛选任务历史并重新打开执行详情；高风险预案可只读查看重复素材主素材建议、引用合并影响、软删除候选风险、学习下载和成交展示位占用，并明确不可执行；草案第一版只记录并执行 `acknowledge_issue` 处理计划，审批和执行前都会校验候选漂移，执行前会抢占执行锁，失败任务保存最近失败原因并可安全重试，已完成任务幂等回放，同一任务并发执行会被拒绝；队列观测可查看最近 job、执行中、失败和可重试压力，学习资料报表可查看章节资料槽位绑定率、资料类型、合规、下载、引用来源和治理问题分布；执行只写批量 `asset_governance` 审计和任务执行结果，不批量修改素材、不合并引用、不软删和不物理删除对象；已通过图片可作为课程详情展示素材，已通过且开启下载且未软删除的章节资料、练习表、音频和视频可绑定到章节素材占位，学习页只对已解锁用户展示受控下载入口；素材治理主面板、治理数据加载和治理弹窗已从课程商品列表迁出到独立工作区，课程详情文案、成交图文、章节资料、素材上传和合规处理已从商品列表迁出到 `/admin/courses/:courseId` 详情页，基础信息、价格、审核和上下架动作弹窗已组件化；后台已新增 `POST /api/catalog/admin/course-products`、`product_create` 审计动作、`/admin/courses/new` 与 `/admin/courses/:courseId/edit` 商品编辑工作台，列表可创建草稿并进入工作台维护基础信息、价格权益、成交主视觉、详情/证明图、摘要、适合人群、成交卖点和结构化 H5 内容块；工作台图片步骤已接入素材库选择器和图片上传，H5 图片块已接入已审核素材选择，发布审核步骤已具备移动成交页预览、预检清单、审核动作、内容质量问题定位、上架/下架动作和发布状态流，商品中心列表行已减负为工作台/发布管理入口，列表行、筛选条、指标区和最近审计摘要已组件化，发布队列只读分组和批量操作预案已接入，前台课程详情已消费后台结构化 H5 内容块；素材对象表、元数据表、引用表、批量任务表、候选快照表、执行明细表、执行审计事件表、执行重试字段和索引已进入迁移草案，下一步进入课程商品发布队列服务端聚合契约与批量草案。
- 已完成商品图片编辑减负：商品工作台图片步骤改为当前主视觉、上传图片、可用图片和当前详情图四段，成交图文图片上传后即可用于主视觉、详情图、H5 图片块和前台课程详情展示；学习资料下载仍保留原合规和权益边界。
- 已完成商品图片排序、批量选择与自动保存：可用图片可勾选多张批量加入详情图，单张设主图/加入详情、上传图片和详情图排序会自动保存，手动标题/说明/URL 修改保留独立保存入口，减少运营在图片步骤反复寻找保存按钮。
- 已完成发布设置去审核化与发布后验证：商品工作台发布页默认收敛为前台售卖状态、移动成交页预览、发布前检查和保存并上架/下架主动作；一键上架先保存当前图文内容，再复用服务端质量校验、兼容状态推进和审计记录；上架后可直接打开前台详情页和课程购买抽屉，减少运营理解审核流和自行查找前台入口的成本。
- 完成课程详情、章节、学习进度、收藏列表。
...
- `/consulting` 咨询预约入口，支持选择咨询师、时段、困扰标签、紧急程度和咨询前说明；当前用户端已升级为咨询师优先的转化页面，提供推荐咨询师选择器、深度资料卡、匹配理由、最近可约时段、右侧预约单、隐私提示和移动端底部预约条，用户可通过咨询师深链直达并在同页完成预约判断。
...
- 用户端已接入咨询待支付召回：`/consulting` 和 `/me?tab=orders` 均可展示待支付咨询预约、支付保留时间和金额，并支持继续支付确认或取消释放时段；个人中心会把咨询预约与课程/会员订单分开展示，避免咨询订单误跳课程入口。
...
- 用户端已接入咨询改期与服务前准备：`/me?tab=orders` 的已预约咨询卡片展示服务前准备、咨询师摘要和“调整时间”入口，改期面板读取当前咨询师可用时段并通过服务端 `reschedule` 动作更新预约。
...
- 接入 `/api/counseling/admin/counselors` 与 `/admin/counseling` 咨询师维护工作台，运营/管理员可在咨询师工作区维护咨询师展示资料、头像、训练背景、咨询风格、适合人群、服务小时数、擅长方向、价格、执业年限、资质摘要、资质状态、资质到期时间、接单开关、接新客门禁和服务状态，并可新增咨询师、软删除名册移除；新增/编辑统一使用右侧抽屉，列表和详情概览不被长表单挤占；详情区可判断前台可见性、预览用户端卡片、打开咨询深链接、查看最近审计详情并定位排班补充时段；前台可预约列表会自动排除暂停接单、关闭接新客、资质待复核、资质过期或已删除的咨询师。
- 用户端咨询预约页已消费后台咨询师档案字段，前台展示头像、资质摘要、训练背景、咨询风格、适合人群、服务小时、匹配标签、价格和可约时段；后台打开的 `/consulting?counselorId=...` 可直接定位该咨询师，用户切换咨询师时同步 URL。
- 个人中心订单页已独立承接咨询预约，咨询预约卡可显示待支付/已预约/已取消等状态、咨询渠道、支付保留时间、金额、继续支付/取消动作、已预约服务前准备和同咨询师改期入口。
...
- 已完成后台壳分离第一刀：后台登录、无权限和授权后工作台不再复用用户端主导航，使用后台专用顶栏、侧边导航、模块上下文、用户端返回入口和后台账号操作。
- 已接入 `/admin/courses` 课程商品列表，课程 seed 可映射为运营商品契约，并支持搜索、状态、审核流、分类、排序、分页、基础信息编辑、内容质量校验、资源级权限、上下架、改价、审计记录、JSON/PostgreSQL 持久化和前台发布联动；详情内容编辑、成交图文、章节/素材占位、素材上传和合规处理已迁入 `/admin/courses/:courseId` 详情页；当前列表默认只加载商品列表和内容质量，素材治理、学习资料报表、批量任务、队列观测和高风险预案已迁入 `/admin/course-assets/governance` 独立工作区，基础信息、价格、审核和上下架弹窗已独立组件化，列表行、筛选条、指标区和最近审计摘要已从课程商品主文件拆出。
...
- 后续继续建立课程商品发布队列服务端聚合契约、批量草案、结算批次、真实渠道适配、通知协作、审计归档读取策略和测评后台。
- 增加运营位、优惠券、会员配置、数据看板。
