...
- 审计中心：`server/modules/audit/auditAdminApi.ts` 负责跨模块审计只读聚合，读取课程商品审计、会员操作审计、订单操作审计、交易操作审计、咨询运营审计和风险复核记录，统一映射为 `AuditCenterEventSchema`；导出和详情复用同一套权限、筛选和隐私边界，导出只生成 CSV 投影，详情只定位来源模块和源事件 ID。统一审计 Store 方案见 `docs/audit-store-architecture.md`，后续归档任务应使用 `AuditCenterArchiveEventSchema` 和 `audit_center_archived_events`，保持只追加、摘要级、幂等回填和可追溯边界。
