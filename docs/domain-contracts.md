...
`CourseCheckoutCreateRequestSchema` 是课程结算创建请求契约，除课程 ID 和结算模式外，可携带账号券包的 `couponClaimId`。服务端会读取 `UserPreferenceSchema` 校验该券已领取、未使用、未过期，并确认对应营销规则仍是当前课程可用的 `course_coupon`；金额仍由服务端课程定价与营销规则统一计算，前端不得自行改写应付金额。`OrderCouponApplicationSchema` 记录订单关联的券包 claim ID、marketing rule ID、预留/已使用状态、关联时间和使用时间；支付成功后课程 checkout service 会把订单券应用推进为 `used`，并同步把账号券包记录写入使用订单和使用时间。用户端结算抽屉可以展示未领取但适用的课程券并调用 `UserPreferenceCouponClaimRequestSchema` 完成领取；领取只更新账号券包和使用意图，订单金额与核销仍以服务端 checkout 为准。个人中心订单深链目前以 `orderId` 查询参数定位到订单列表中的具体订单，后续订单详情页应继续复用同一 `Order` 与 `couponApplication` 契约。
