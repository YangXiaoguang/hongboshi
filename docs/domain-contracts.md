...
- 咨询预约：`server/modules/counseling/counselingAppointmentStore.ts` 负责时段、预约单和预约关联风险事件；运营排班动作复用同一 slot 存储，关闭时段表示为 `available=false` 且无活跃预约。
- 咨询运营：`server/modules/counseling/counselingOperationStore.ts` 负责取消规则以及规则、排班、履约审计。
