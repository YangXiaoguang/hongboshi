...
| `auditCenter.ts`      | 统一审计中心只读聚合、筛选、分页和隐私边界                       |
...
`AuditCenterListResultSchema` 是统一审计中心第一版只读聚合契约，从课程商品审计、会员操作审计、订单操作审计、交易操作审计、咨询运营审计和风险复核记录中归一化 `AuditCenterEventSchema`。列表支持模块、动作、操作者、资源关键词、日期范围和分页筛选；事件只包含模块、动作、资源定位、操作者、原因、摘要、发生时间和 before/after 摘要，不新增写入真相源，不允许审计事件修改或删除，也不暴露咨询说明、测评答案、风险信号原文和支付敏感原文。后续导出、事件详情和统一审计 Store 应继续复用该只读契约向前兼容。
...
- 审计中心：`server/modules/audit/auditAdminApi.ts` 负责跨模块审计只读聚合，读取课程商品审计、会员操作审计、订单操作审计、交易操作审计、咨询运营审计和风险复核记录，统一映射为 `AuditCenterEventSchema`；该模块不写入业务 Store，不修改审计事实，后续导出和事件详情应继续保持只读边界。
