...
- 财务后台：`server/modules/finance/financeAdminApi.ts` 负责收入、退款、净收款、待退款和异常金额只读聚合，读取支付回调收据、课程/会员/咨询订单、交易异常工单和 auth 用户目录；财务口径由服务端统一计算，前端只展示聚合结果和口径说明。
