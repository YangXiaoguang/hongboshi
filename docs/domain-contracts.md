...
- 课程素材治理任务：`server/modules/catalog/courseProductAssetGovernanceBatchTaskStore.ts` 负责保存批量治理任务草案，当前支持内存和 JSON 文件 `.hongboshi-data/course-product-asset-governance-batch-tasks.json`。第一版任务只保存 `acknowledge_issue` 待审批草案、审批/驳回记录、审批前预检和取消记录，不执行素材批量写入；后续执行切片应在该 Store 或 PostgreSQL 等价 Store 上扩展执行预案、执行状态机和批量审计计划。
