...
课程 `refund.succeeded` 回调保存 `refunded` 订单时，会通过 `settleRefundedCourseAccessOrder` 结清单课权益：先保留退款完成订单事实，再按课程 ID 检查是否仍有其他 `paid/refunding` 同课订单；只有不存在其他有效同课订单时，才从 `ownedCourseIds` 移除该课程。用户端课程列表、课程详情和成长空间继续只依赖 `resolveCourseAccess` 计算可学习状态，已退款单课会回到购买入口；个人中心已退款订单只展示退款完成、权益已停止和重新购买，不再提供继续学习入口。会员订单退款暂不自动关闭 `membership`，因为当前会员状态缺少来源订单字段；下一阶段需补齐会员来源后再做安全回收。
