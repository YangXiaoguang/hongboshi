...
当前后台权限已经开始从粗粒度 `admin:manage` 拆分为资源级能力。课程商品模块使用 `catalog:read`、`catalog:edit`、`catalog:review`、`catalog:publish`、`catalog:price` 控制列表/详情读取、内容编辑、审核、上下架和改价；用户会员后台使用 `user:read` 控制用户列表与详情只读聚合；`catalog_viewer` 为课程商品只读角色，`catalog_operator` 为课程商品运营角色，`operator` 与 `admin` 继续拥有课程商品完整权限，且拥有用户后台读取权限。

`UserAdminListResultSchema` 与 `UserAdminDetailSchema` 是运营用户会员后台的只读契约，只暴露账号摘要、角色、会员状态、课程权益、订单摘要、咨询预约摘要和风险等级/状态摘要。手机号只允许脱敏值，咨询说明、测评答案和风险信号原文不进入该契约；后续需要查看敏感内容时应新增更高权限和审计事件，而不是扩展当前只读视图。
