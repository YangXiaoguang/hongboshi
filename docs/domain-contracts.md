...
`RiskAdminListResultSchema` 与 `RiskAdminDetailSchema` 是风险复核台的后台投影契约，聚合 `riskEventStore` 中的风险事件、auth 用户脱敏摘要、关联测评/咨询对象摘要和人工处理记录。列表支持风险等级、状态、来源、关键词、排序和分页；详情只输出复核所需 `signalSummary`、关联对象摘要、SOP 提醒和处理记录，不输出 `RiskEvent.signal`、测评答案原文或咨询前说明全文。`RiskAdminActionRequestSchema`、`RiskAdminReviewRecordSchema` 与 `RiskAdminMutationResultSchema` 描述开始复核、已联系用户、建议咨询、升级处理和标记解决动作，记录操作者、角色、前后状态、备注和处理时间。处理记录当前由独立 `RiskReviewStore` 保存，避免把审计事实混入风险事件本体。
...
- 风险复核：`server/modules/risk/riskAdminApi.ts` 负责风险事件队列、隐私最小化详情和受控处理动作，读取风险事件 Store、auth 用户目录、测评结果、咨询预约和 `RiskReviewStore`。`server/modules/risk/riskReviewStore.ts` 当前支持内存和 JSON 文件 `.hongboshi-data/risk-reviews.json`，处理记录只保存处理摘要、操作者、角色、动作、前后状态和时间。
