...
当前后台权限已经开始从粗粒度 `admin:manage` 拆分为资源级能力。课程商品模块使用 `catalog:read`、`catalog:edit`、`catalog:review`、`catalog:publish`、`catalog:price` 控制列表/详情读取、内容编辑、审核、上下架和改价；用户会员后台使用 `user:read` 控制用户列表与详情聚合，使用 `user:membership` 控制会员开通、延期、到期标记和计划调整；订单后台使用 `order:read` 控制课程、会员和咨询订单列表/详情读取，使用 `order:operate` 控制关闭待支付订单、标记异常和解除异常；`catalog_viewer` 为课程商品只读角色，`catalog_operator` 为课程商品运营角色，`operator` 与 `admin` 继续拥有课程商品完整权限，且拥有用户后台读取、会员操作、订单后台读取和订单操作权限。
