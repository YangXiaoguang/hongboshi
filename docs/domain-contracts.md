...
- 风险复核：`server/modules/risk/riskAdminApi.ts` 负责风险事件队列、隐私最小化详情、SOP 模板匹配、升级队列和受控处理动作，读取风险事件 Store、auth 用户目录、测评结果、咨询预约、`RiskReviewStore` 和 `RiskSopStore`。`server/modules/risk/riskReviewStore.ts` 当前支持内存和 JSON 文件 `.hongboshi-data/risk-reviews.json`，处理记录只保存处理摘要、操作者、角色、动作、前后状态、SOP 模板版本、升级摘要和时间。`server/modules/risk/riskSopStore.ts` 当前支持内存和 JSON 文件 `.hongboshi-data/risk-sop.json`，保存默认 SOP 模板、模板启停、结果模板和升级队列。
