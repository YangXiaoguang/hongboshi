...
- 课程 seed 来自 `shared/data/mockCourses.ts`，当前用于课程商品 Store 初始化和测试 fallback；前台课程列表/详情通过课程商品 Store 读取已审核通过且已上架商品，课程路径、快速开始区和课程列表卡片已支持立即购买/开通会员/开始学习/继续支付的主动作，`/courses` 可直接打开半屏结算抽屉，待支付课程订单会在课程中心、课程详情和成长空间召回，课程详情页已前置信任模块展示讲师、学习反馈、售后/隐私边界和 FAQ
