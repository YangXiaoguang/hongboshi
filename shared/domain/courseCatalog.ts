...
export type CourseCatalogQuery = CourseListQuery;

export const CourseCatalogResultSchema = z.object({
  items: z.array(CourseSchema),
  paginatedItems: z.array(CourseSchema),
...
export type CourseCatalogResult = z.infer<typeof CourseCatalogResultSchema>;
...
export function listCoursesByQuery(
  ...
  const parsedQuery = CourseListQuerySchema.parse(query);
  const items = filterCourses(courses, parsedQuery);
  const pageResult = paginateCourses(items, parsedQuery.page, parsedQuery.pageSize);

  return CourseCatalogResultSchema.parse({
    items,
